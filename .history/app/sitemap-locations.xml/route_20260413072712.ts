...
const transmissions = [
  {
    ...
    cons: ['Not ideal if you are unused to driving abroad', 'Can be tiring in heavy traffic'],
  },
  {
    type: 'Automatic',
    emoji: '🤖',
    desc: 'Increasingly common, especially in the USA, Canada and UAE. Easier to drive, particularly in busy cities or on unfamiliar roads. Worth specifying when you search if this is your preference.',
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {carCategories.map((cat) => (
              <div key={cat.label} className="bg-gray-50 border border-gray-100 rounded-2xl p-6 hover:shadow-md transition">
                ...
                <p className="text-sm text-gray-700 mb-3">{cat.best}</p>
      ...
      <section className="py-16 px-6 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
            {transmissions.map((t) => (
              <div key={t.type} className="bg-white rounded-2xl border border-gray-200 p-7 shadow-sm">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-3xl">{t.emoji}</span>
                  <h3 className="font-bold text-xl" style={{ color: '#232e4e' }}>{t.type}</h3>
                </div>
                <p className="text-sm text-gray-600 mb-5 leading-relaxed">{t.desc}</p>
