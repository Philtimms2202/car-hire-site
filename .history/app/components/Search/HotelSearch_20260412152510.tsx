...
export default function HotelSearch() {
  ...
  const handleSearch = () => {
    ...
    // ✔ Adults always included per room
    params.append(
      'adults',
      rooms.map(r => r.adults).join(',')
    )

    // ✔ ONLY include rooms that actually have children
    const childrenPerRoom = rooms
      .map(r => r.children)
      .filter(c => c > 0)

    if (childrenPerRoom.length > 0) {
      params.append(
        'children',
        childrenPerRoom.join(',')
      )
    }
