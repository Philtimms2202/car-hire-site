...
<div className="flex items-end gap-2">
  ...
  {/* Swap button */}
  <button
    className="mb-[1px] flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full border border-gray-300 bg-white text-gray-600 hover:bg-gray-100 transition"
    onClick={() => {
      const tempVal = from
...
<div className="relative" ref={travellerRef}>
  ...
  {travellerOpen && (
    <div className="absolute left-0 right-0 z-40 bg-white border border-gray-200 rounded-xl shadow-xl mt-2 p-4 space-y-4">
      ...
      <button className="btn-primary w-full mt-2" onClick={() => setTravellerOpen(false)}>Apply</button>
    </div>
  )}
</div>
