...
export default function HotelSearch() {
  ...
  const updateRoom = (
    ...
    // keep age array aligned
    if (field === 'children') {
      const diff = value - updated[index].ages.length
      if (diff > 0) {
        updated[index].ages.push(...Array(diff).fill(7))
      } else {
  ...
  const handleSearch = () => {
    ...
    // =========================
    // 🔥 FIXED CHILD STRATEGY
    // =========================
    // Key idea:
    // NEVER allow Expedia to re-map children between rooms
    // so we always enforce stable slot encoding
    ...
    const childrenParam = rooms
      .map(room => {
        ...
        const ages = Array(MAX_CHILDREN_PER_ROOM)
          .fill(0)
          .map((_, i) => room.ages?.[i] ?? 7)
                  ...
                  <div key={i} className="mb-4 border-b pb-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() =>
                          updateRoom(i,'adults',Math.max(1,room.adults-1))
                        }>-</button>
                        <span>{room.adults}</span>
                        <button onClick={() =>
                          updateRoom(i,'adults',room.adults+1)
                        }>+</button>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between mt-2">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() =>
                          updateRoom(i,'children',Math.max(0,room.children-1))
                        }>-</button>
                        <span>{room.children}</span>
                        <button onClick={() =>
                          updateRoom(i,'children',Math.min(6,room.children+1))
                        }>+</button>
                      </div>
                    </div>
