...
const handleSearch = () => {
  ...
  // Kiwi uses YYYY-MM-DD, so no need for your DDMM formatter
  const departDate = depart
  const returnDateFormatted = returnDate || ''

  // Build Kiwi deeplink
  const base = `https://www.kiwi.com/en/search/results/${fromCode}/${toCode}/${departDate}`

  const url = new URL(
    roundTrip && returnDateFormatted
      ? `${base}/${returnDateFormatted}`
      : base
  )

  // Required: your Travelpayouts Kiwi affiliate ID
  url.searchParams.set('affilid', process.env.NEXT_PUBLIC_KIWI_AFFIL_ID || '')

  // Optional: passengers, cabin, etc.
