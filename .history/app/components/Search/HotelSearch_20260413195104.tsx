...
// ─────────────────────────────────────────────
// EXPEDIA TRACKING HELPER
// ✅ Routes through prf.hn/click so Partnerize injects the clickref
//    token required for clicks and conversions to be attributed to you.
//
// Your camref is the ID segment from your affcid: 1011l428377
// Full affcid was: UK.DIRECT.PHG.1011l428377
// ─────────────────────────────────────────────
const CAMREF = '1011l428377' // Your Partnerize camref

/**
 * Wraps any Expedia destination URL in the Partnerize tracking redirect.
 * All clicks made through this link will appear in your Creator Dashboard.
 */
function buildTrackedExpediaUrl(destinationUrl: string): string {
  // prf.hn/click/camref:{camref}/destination:{url}
  // The destination URL must be fully encoded
  return `https://prf.hn/click/camref:${CAMREF}/destination:${encodeURIComponent(destinationUrl)}`
}
...
export default function HotelSearch() {
  ...
  const handleSearch = () => {
    ...
    // 1. Build the raw Expedia search URL (no affcid — tracking handled by prf.hn)
    ...
    const expediaUrl = `${baseUrl}/Hotel-Search?${params.toString()}`

    // 2. Wrap in Partnerize redirect so clicks are attributed to your account
    const trackedUrl = buildTrackedExpediaUrl(expediaUrl)
