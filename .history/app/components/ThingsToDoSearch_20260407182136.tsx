...
export default function ThingsToDoSearch({ cityName }: { cityName: string }) {
  ...
  return (
    <>
      ...
      {/* Search Box */}
      <div className="max-w-2xl mx-auto flex gap-3 mt-10 px-4">
        <input
          type="text"
          value={search}
        ...
        <button
          ...
          className="btn-primary px-6 py-4 rounded-xl font-semibold whitespace-nowrap"
        >
          Search
        </button>
      </div>
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className="whitespace-nowrap px-4 py-2 rounded-full text-sm font-semibold border transition flex-shrink-0"
                style={
                  isActive
                    ? { backgroundColor: '#2f797c', color: 'white', borderColor: '#2f797c' }
                    : { backgroundColor: 'white', color: '#232e4e', borderColor: '#e5e7eb' }
                }
              >
                {cat.label}
              </button>
