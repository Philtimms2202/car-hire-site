...
const hotelTypes = [
  ...
  { label: 'Boutique',   icon: '◈', desc: 'Unique, character-rich stays',  propertyType: 'BED_AND_BREAKFAST' },
  { label: 'Budget',     icon: '◇', desc: 'Great value, no compromises',   propertyType: 'HOSTEL' },
  { label: 'Apartments', icon: '⬡', desc: 'Home comforts, longer stays',   propertyType: 'APART_HOTEL' },
]

// Build URL (with optional star rating for Luxury)
const expediaTypeUrl = (propertyType: string, starRating?: string) => {
  const params = new URLSearchParams()
  params.set('affcid', 'UK.DIRECT.PHG.1011l428377')
...
<section className="py-14 px-6 bg-gray-50">
  <div className="max-w-5xl mx-auto">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {hotelTypes.map(({ label, icon, desc, propertyType, starRating }) => (
        
          key={label}
          href={expediaTypeUrl(propertyType, starRating)}
          target="_blank"
        ...
        >
          <div className="text-2xl mb-2 transition-transform duration-200 group-hover:scale-110" style={{ color: '#2f797c' }}>
            {icon}
          </div>
