...
const expediaCityUrl = (city: string) =>
  `${EXPEDIA_BASE}/Hotel-Search?${EXPEDIA_AFFILIATE}&destination=${encodeURIComponent(city)}`
...
function DestinationCard({ dest }: { dest: typeof destinations[0] }) {
  const url = `https://www.trip.com/hotels/${dest.tripCity}/?${AFFILIATE}`
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              { n: 3, title: 'Book', text: 'Secure your stay directly — no hidden fees, and free cancellation on most rooms.' },
            ].map(({ n, title, text }) => (
      ...
      <section className="py-16 px-6">
        ...
        >
          ...
          <p className="text-teal-100 mb-6 text-sm max-w-md mx-auto">
            Browse our full hotel catalogue on Trip.com and discover deals across every destination.
          </p>
          <a
            href={`https://www.trip.com/hotels/?${AFFILIATE}`}
            target="_blank"
            rel="noopener noreferrer"
