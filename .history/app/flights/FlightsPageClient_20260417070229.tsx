  ...
  return (
    <>
      <style>{`
        ...
        .tt { font-family: 'DM Sans', sans-serif; color: var(--text); background: #fff; }

        /* HERO */
        .tt-hero { background: var(--navy); position: relative; overflow: hidden; }
        .tt-hero::before {
          content: ''; position: absolute; inset: 0; pointer-events: none;
          background:
            radial-gradient(ellipse 80% 60% at 15% 60%, rgba(212,137,26,0.14) 0%, transparent 60%),
            radial-gradient(ellipse 50% 70% at 85% 10%, rgba(255,255,255,0.04) 0%, transparent 50%);
        }
        .tt-hero::after {
          content: ''; position: absolute; inset: 0; pointer-events: none;
        ...
        .tt-hero-in { position: relative; z-index: 10; max-width: 1160px; margin: 0 auto; padding: 80px 24px 76px; }
        .tt-eyebrow { display: inline-block; font-size: 11px; font-weight: 600; letter-spacing: .22em; text-transform: uppercase; color: var(--amb2); margin-bottom: 20px; }
        .tt-hero h1 { font-family: 'Playfair Display', Georgia, serif; font-size: clamp(2.4rem,5vw,3.8rem); font-weight: 900; line-height: 1.12; color: #fff; max-width: 700px; margin: 0 0 18px; }
        .tt-hero h1 em { font-style: normal; color: var(--amb2); }
        .tt-hero-sub { color: rgba(255,255,255,.6); font-size: 1.05rem; font-weight: 300; max-width: 540px; margin: 0 0 36px; line-height: 1.7; }

        .tt-tabs { display: flex; gap: 4px; background: rgba(255,255,255,.1); border-radius: 14px; padding: 4px; max-width: 420px; margin-bottom: 22px; }
        .tt-tab { flex: 1; padding: 9px 10px; border-radius: 10px; font-size: 12px; font-weight: 600; cursor: pointer; border: none; background: transparent; color: rgba(255,255,255,.6); transition: all .2s; font-family: 'DM Sans',sans-serif; white-space: nowrap; }
        .tt-tab:hover { color: #fff; }
        .tt-tab.on { background: #fff; color: var(--navy); box-shadow: 0 2px 8px rgba(0,0,0,.15); }

        .tt-sbox { background: #fff; border-radius: 20px; padding: 28px; max-width: 900px; box-shadow: 0 24px 64px rgba(0,0,0,.3); }

        .tt-trust { display: flex; gap: 28px; margin-top: 28px; flex-wrap: wrap; }
        .tt-trust-item { display: flex; align-items: center; gap: 8px; color: rgba(255,255,255,.52); font-size: 13px; }
        .tt-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--amber); flex-shrink: 0; }

        /* SHARED SECTION */
        .tt-sec { padding: 80px 24px; }
        .tt-sec-in { max-width: 1100px; margin: 0 auto; }
        .tt-label { font-size: 11px; font-weight: 600; letter-spacing: .2em; text-transform: uppercase; color: var(--amber); margin-bottom: 10px; }
        .tt-h2 { font-family: 'Playfair Display',Georgia,serif; font-size: clamp(1.7rem,3vw,2.4rem); font-weight: 700; color: var(--navy); margin: 0 0 10px; line-height: 1.2; }
        .tt-sub { color: var(--muted); font-size: 1rem; font-weight: 300; max-width: 580px; line-height: 1.7; margin: 0 0 44px; }
        .tt-cream { background: var(--cream); }

        /* AIRPORT SELECTOR */
        .tt-ap-wrap { position: relative; max-width: 420px; margin-bottom: 40px; }
        .tt-ap-wrap label { display: block; font-size: 11px; font-weight: 600; letter-spacing: .1em; text-transform: uppercase; color: var(--muted); margin-bottom: 8px; }
        .tt-ap-input { width: 100%; padding: 12px 16px; border-radius: 10px; border: 1.5px solid var(--stone); font-family: 'DM Sans',sans-serif; font-size: 14px; color: var(--navy); background: #fff; outline: none; transition: border-color .2s; }
        .tt-ap-input:focus { border-color: var(--amber); }
        .tt-ap-drop { position: absolute; top: calc(100% + 6px); left: 0; right: 0; background: #fff; border: 1.5px solid var(--stone); border-radius: 14px; box-shadow: 0 12px 40px rgba(0,0,0,.12); z-index: 50; max-height: 300px; overflow-y: auto; }
        .tt-ap-geo { width: 100%; text-align: left; padding: 12px 16px; border: none; border-bottom: 1px solid var(--stone); background: transparent; cursor: pointer; font-family: 'DM Sans',sans-serif; display: flex; align-items: center; justify-content: space-between; font-size: 13px; transition: background .15s; }
        .tt-ap-geo:hover { background: #fffbf5; }
        .tt-ap-row { padding: 10px 16px; cursor: pointer; transition: background .15s; border-bottom: 1px solid #f3f3f3; }
        .tt-ap-row:hover { background: #fffbf5; }
        .tt-ap-top { display: flex; justify-content: space-between; align-items: center; }
        .tt-ap-name { font-weight: 600; font-size: 13px; color: var(--navy); }
        .tt-ap-iata { font-weight: 700; font-size: 12px; color: var(--amber); }
        .tt-ap-sub { font-size: 12px; color: var(--muted); margin-top: 2px; }

        /* DEST CARDS */
        .tt-dest-grid { display: grid; grid-template-columns: repeat(auto-fill,minmax(300px,1fr)); gap: 18px; }
        .tt-dest-card { background: var(--cream); border-radius: 18px; padding: 24px; border: 1.5px solid var(--stone); transition: box-shadow .25s,transform .25s,border-color .25s; }
        .tt-dest-card:hover { box-shadow: 0 14px 40px rgba(26,36,64,.1); transform: translateY(-3px); border-color: var(--amber); }
        .tt-dest-top { display: flex; align-items: flex-start; gap: 14px; margin-bottom: 12px; }
        .tt-dest-emoji { font-size: 2.2rem; line-height: 1; flex-shrink: 0; }
        .tt-dest-city { font-family: 'Playfair Display',Georgia,serif; font-size: 1.2rem; font-weight: 700; color: var(--navy); line-height: 1.2; }
        .tt-dest-cty { font-size: 12px; color: var(--muted); margin-top: 2px; }
        .tt-dest-tag { font-size: 13px; color: var(--muted); line-height: 1.55; margin-bottom: 18px; font-weight: 300; }
        .tt-link { display: inline-flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 600; color: var(--amber); background: none; border: none; cursor: pointer; padding: 0; font-family: 'DM Sans',sans-serif; transition: gap .2s,color .2s; }
        .tt-link:hover { color: var(--navy); gap: 10px; }
        .tt-link:disabled { color: var(--muted); cursor: not-allowed; }

        /* ROUTES */
        .tt-routes { display: grid; grid-template-columns: repeat(auto-fill,minmax(240px,1fr)); gap: 14px; }
        .tt-route { background: #fff; border: 1.5px solid var(--stone); border-radius: 14px; padding: 18px 20px; display: flex; align-items: center; gap: 14px; transition: border-color .2s,box-shadow .2s,transform .2s; cursor: pointer; text-align: left; width: 100%; }
        .tt-route:hover { border-color: var(--amber); box-shadow: 0 6px 20px rgba(0,0,0,.08); transform: translateY(-2px); }
        .tt-route-flag { font-size: 1.8rem; flex-shrink: 0; }
        .tt-route-info { flex: 1; min-width: 0; }
        .tt-route-path { font-weight: 600; font-size: 14px; color: var(--navy); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .tt-route-codes { font-size: 11px; color: var(--muted); margin-top: 2px; }
        .tt-route-dur { font-size: 12px; font-weight: 600; color: var(--amber); white-space: nowrap; }

        /* SEASONAL */
        .tt-season-block { margin-bottom: 56px; }
        .tt-season-block:last-child { margin-bottom: 0; }
        .tt-season-hd { display: flex; align-items: baseline; gap: 14px; margin-bottom: 4px; }
        .tt-season-title { font-family: 'Playfair Display',Georgia,serif; font-size: 1.55rem; font-weight: 700; color: var(--navy); }
        .tt-season-mo { font-size: 11px; font-weight: 600; letter-spacing: .1em; text-transform: uppercase; color: var(--muted); }
        .tt-season-desc { font-size: 14px; color: var(--muted); font-weight: 300; line-height: 1.65; max-width: 560px; margin: 0 0 18px; }
        .tt-season-grid { display: grid; grid-template-columns: repeat(auto-fill,minmax(280px,1fr)); gap: 14px; }
        .tt-s-card { background: #fff; border-radius: 14px; padding: 22px; border-left: 4px solid var(--sc,var(--amber)); box-shadow: 0 2px 10px rgba(0,0,0,.05); transition: box-shadow .2s,transform .2s; }
        .tt-s-card:hover { box-shadow: 0 8px 28px rgba(0,0,0,.1); transform: translateY(-2px); }
        .tt-s-city { font-family: 'Playfair Display',Georgia,serif; font-size: 1.1rem; font-weight: 700; color: var(--navy); margin-bottom: 1px; }
        .tt-s-country { font-size: 12px; color: var(--muted); margin-bottom: 10px; }
        .tt-s-hl { font-size: 12px; font-style: italic; color: var(--sc,var(--amber)); margin-bottom: 14px; font-weight: 500; }
        .tt-s-meta { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 6px; margin-bottom: 16px; }
        .tt-meta { background: var(--cream); border-radius: 8px; padding: 8px 6px; text-align: center; }
        .tt-meta-l { font-size: 9px; font-weight: 600; letter-spacing: .1em; text-transform: uppercase; color: var(--muted); margin-bottom: 3px; }
        .tt-meta-v { font-size: 12px; font-weight: 600; color: var(--navy); }

        /* TIPS */
        .tt-tips { display: grid; grid-template-columns: repeat(auto-fill,minmax(300px,1fr)); gap: 20px; }
        .tt-tip { background: #fff; border-radius: 16px; padding: 26px; border: 1.5px solid var(--stone); transition: border-color .2s,box-shadow .2s; }
        .tt-tip:hover { border-color: var(--amber); box-shadow: 0 6px 24px rgba(0,0,0,.07); }
        .tt-tip-icon { font-size: 1.8rem; margin-bottom: 14px; }
        .tt-tip-title { font-family: 'Playfair Display',Georgia,serif; font-size: 1.05rem; font-weight: 700; color: var(--navy); margin-bottom: 8px; }
        .tt-tip-body { font-size: 14px; font-weight: 300; color: var(--muted); line-height: 1.7; }

        /* INFO STRIP */
        .tt-strip { background: var(--navy); padding: 64px 24px; }
        .tt-strip-in { max-width: 1100px; margin: 0 auto; display: grid; grid-template-columns: repeat(auto-fit,minmax(220px,1fr)); gap: 40px; }
        .tt-strip-icon { font-size: 1.8rem; margin-bottom: 14px; }
        .tt-strip-title { font-family: 'Playfair Display',Georgia,serif; font-size: 1.05rem; font-weight: 700; color: #fff; margin-bottom: 8px; }
        .tt-strip-body { font-size: 13.5px; font-weight: 300; color: rgba(255,255,255,.52); line-height: 1.65; }

        /* PROSE */
        .tt-prose-sec { padding: 80px 24px; background: #fff; }
        .tt-prose-in { max-width: 880px; margin: 0 auto; }
        .tt-prose-in h2 { font-family: 'Playfair Display',Georgia,serif; font-size: clamp(1.6rem,3vw,2rem); font-weight: 700; color: var(--navy); margin: 0 0 16px; line-height: 1.25; }
        .tt-prose-in h3 { font-family: 'Playfair Display',Georgia,serif; font-size: 1.2rem; font-weight: 700; color: var(--navy); margin: 36px 0 10px; }
        .tt-prose-in p { font-size: 15px; font-weight: 300; color: #4b5563; line-height: 1.85; margin: 0 0 18px; }
        .tt-prose-in strong { font-weight: 600; color: var(--navy); }
        .tt-hr { border: none; border-top: 1px solid var(--stone); margin: 48px 0; }
        .tt-cols { display: grid; grid-template-columns: 1fr 1fr; gap: 52px; margin-top: 48px; }
        @media(max-width:680px){ .tt-cols { grid-template-columns:1fr; } }

        /* FAQ */
        .tt-faq-sec { background: var(--cream); padding: 80px 24px; }
        .tt-faq-in { max-width: 780px; margin: 0 auto; }
        .tt-faq-item { border-bottom: 1px solid var(--stone); }
        .tt-faq-btn { width: 100%; text-align: left; padding: 22px 0; background: none; border: none; cursor: pointer; display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; font-family: 'DM Sans',sans-serif; }
        .tt-faq-q { font-weight: 600; font-size: 15px; color: var(--navy); line-height: 1.45; text-align: left; }
        .tt-chev { font-size: 18px; color: var(--amber); flex-shrink: 0; transition: transform .25s; margin-top: 2px; }
        .tt-chev.open { transform: rotate(180deg); }
        .tt-faq-a { font-size: 14px; font-weight: 300; color: var(--muted); line-height: 1.75; padding-bottom: 22px; max-width: 680px; }

        /* CTA */
        .tt-cta { background: var(--amber); padding: 64px 24px; text-align: center; }
        .tt-cta h2 { font-family: 'Playfair Display',Georgia,serif; font-size: clamp(1.6rem,3vw,2.2rem); font-weight: 700; color: var(--navy); margin: 0 0 12px; }
        .tt-cta p { font-size: 15px; color: rgba(26,36,64,.72); font-weight: 300; max-width: 480px; margin: 0 auto 28px; line-height: 1.65; }
        .tt-cta-btn { display: inline-block; background: var(--navy); color: #fff; font-family: 'DM Sans',sans-serif; font-size: 14px; font-weight: 600; padding: 14px 32px; border-radius: 10px; border: none; cursor: pointer; transition: background .2s,transform .15s; }
        .tt-cta-btn:hover { background: #0f1729; transform: translateY(-1px); }

        @media(max-width:640px){
          .tt-hero-in { padding: 56px 20px 56px; }
          .tt-tabs { max-width: 100%; }
          .tt-trust { gap: 14px; }
          .tt-sec { padding: 56px 20px; }
        }
      `}</style>
