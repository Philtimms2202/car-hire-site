...
const popularRoutes: PopularRoute[] = [
  ...
  {
    from: 'London',  fromIata: 'LGW',
    to: 'Orlando',   toIata: 'MCO',
    emoji: '✈️', flightTime: '~9h', priceFrom: 'From £449',
    internalSlug: 'london-to-orlando',
  },
]
...
export default function FlightsPageClient() {
  ...
  const openPopularRoute = (route: PopularRoute) => {
    if (route.internalSlug) {
      window.location.href = `/popular-routes/${route.internalSlug}`
    } else {
      const url = buildTrackedKiwiUrl({
        from: route.fromIata,
        to: route.toIata,
