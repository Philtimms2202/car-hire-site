...
function searchAirports(
  ...
  return airports
    .map(a => {
      ...
      if (city.startsWith(q)) score += 70

      // Contains (fallback)
      if (city.includes(q)) score += 50
      if (name.includes(q)) score += 40
      if (iata.includes(q)) score += 30
      if (country.includes(q)) score += 10

      return { airport: a, score }
    })
    // 🔥 KEY CHANGE: don’t require score > 0 strictly
    .filter(r => r.score > 0)
...
function AirportInput({
  ...
  useEffect(() => {
    ...
    // Only open if we actually have results
      ...
      {open && suggestions.length > 0 && (
        <ul className="absolute z-50 top-full mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
          {suggestions.map((a) => (
            ...
            >
              <div>
                <span className="font-medium" style={{ color: '#022135' }}>
                  {a.city}
                </span>
...
export default function FlightTimeCalculator() {
  ...
  fetch('/data/airports.json')
    .then(r => r.json())
    .then((data: Airport[]) => {
      console.log('RAW airports loaded:', data.length)

      const filtered = data.filter(
        a => a.iata_code && a._geoloc?.lat && a._geoloc?.lng
      )

      console.log('FILTERED airports:', filtered.length)
      console.log('SAMPLE airport:', filtered[0])
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
  ...
  <AirportInput
    ...
    onChange={(v) => {
      ...
      const match = airports.find(a =>
        v.toLowerCase().includes(a.iata_code?.toLowerCase()) ||
        v.toLowerCase().includes(a.city?.toLowerCase())
      )

      setOriginAirport(match || null)
    }}
    onSelect={(a) => {
      setOriginAirport(a)
      setOriginText(`${a.city} (${a.iata_code})`)
    }}
  ...
  <AirportInput
    ...
    onChange={(v) => {
      ...
      setDestAirport(match || null)
    }}
    onSelect={(a) => {
      setDestAirport(a)
      setDestText(`${a.city} (${a.iata_code})`)
    }}
    placeholder="City name or IATA code"
  />
    ...
    <div className="flex-1">
      <AirportInput
        ...
        onChange={(v) => {
          ...
          updateStopAirport(i, match || null)
        }}
        onSelect={(a) => {
          updateStopAirport(i, a)
          updateStopText(i, `${a.city} (${a.iata_code})`)
        }}
        placeholder="City name or IATA code"
      />
    </div>
  ...
  <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-6 space-y-6">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
        ...
        <p className="text-lg font-semibold" style={{ color: '#022135' }}>
          {conv(result.totalKm).toLocaleString()} {sym}
        </p>
      </div>
      ...
      <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
        ...
        <p className="text-lg font-semibold" style={{ color: '#022135' }}>
          {fmtTime(Math.round(result.totalFlightMins))}
        </p>
      </div>

      {stopAirports.filter(Boolean).length > 0 && (
        <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
          <p className="text-xs text-gray-500 mb-1">Total journey</p>
          <p className="text-lg font-semibold" style={{ color: '#022135' }}>
            {fmtTime(Math.round(result.totalWithLayovers))}
          </p>
        </div>
      )}
    ...
    <div className="space-y-2">
      ...
      {result.legs.map((leg, i) => (
        <div
          key={i}
          className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 flex justify-between items-center"
        >
          <div>
    ...
    <div className="flex flex-col md:flex-row gap-3 pt-2">
      ...
      <a
        href={`https://flights.timmstravel.com/search?from=${originAirport?.iata_code}&to=${destAirport?.iata_code}`}
        target="_blank"
        className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white text-center transition hover:opacity-90"
    ...
    <p className="text-xs text-gray-400 pt-1 leading-relaxed">
      Estimated using great-circle distance (haversine formula) at an average
      cruising speed of 900 km/h. Actual flight times may vary due to aircraft
      type, routing and wind conditions.
    </p>
