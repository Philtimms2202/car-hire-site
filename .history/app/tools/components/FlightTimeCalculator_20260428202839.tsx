  ...
  <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 space-y-5">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {[
        {
          label: 'Distance',
          value: `${conv(result.totalKm).toLocaleString()} ${sym}`,
        },
        {
          label: 'Flight time',
          value: fmtTime(Math.round(result.totalFlightMins)),
        },
        ...(stopAirports.filter(Boolean).length
          ? [{
              label: 'With layovers',
              value: fmtTime(Math.round(result.totalWithLayovers)),
            }]
          : []),
        {
          label: 'Legs',
          value: String(result.legs.length),
        },
      ].map(stat => (
        <div
          key={stat.label}
    ...
    <div className="flex flex-col md:flex-row gap-3 pt-2">
      ...
      {originAirport && destAirport ? (
        <a
          href={`https://flights.timmstravel.com/search?from=${encodeURIComponent(
            originAirport.iata_code
          )}&to=${encodeURIComponent(destAirport.iata_code)}`}
          target="_blank"
        ...
        <div className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white text-center bg-gray-300 cursor-not-allowed">
          Search flights
        </div>
      )}
    </div>
