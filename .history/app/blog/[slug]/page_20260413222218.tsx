...
// ======================================================
// Fetch Blog Post (single source of truth)
// ======================================================
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  // If for any reason params.slug is missing, avoid querying with $slug
  if (!params?.slug) {
    return {
  ...
  const ogImage = post.metaImage ? urlFor(post.metaImage) : null
        ...
        {post.body && (
          <PortableText
            ...
            components={{
              block: {
                normal: ({ children }) => (
                  <p className="text-gray-600 leading-8 mb-6">{children}</p>
                ),
                h2: ({ children }) => (
                  <h2
                ...
                blockquote: ({ children }) => (
                  <blockquote
                    className="border-l-4 pl-4 italic text-gray-500 my-6"
                    style={{ borderColor: '#2f797c' }}
                  >
                    {children}
                  </blockquote>
                ),
              ...
              list: {
                bullet: ({ children }) => (
                  <ul className="list-disc list-inside mb-6 text-gray-600 leading-8">
                    {children}
                  </ul>
                ),
                number: ({ children }) => (
                  <ol className="list-decimal list-inside mb-6 text-gray-600 leading-8">
                    {children}
                  </ol>
                ),
              },
