...
async function getPosts(page: number, category?: string) {
  ...
  return { posts, total, categories: (categories as string[]).filter(Boolean) }
}

function buildUrl(p: number, cat?: string): string {
      ...
      <section className="border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-6 py-4 flex gap-2 flex-wrap">
          ...
          {/* All button */}
            ...
            style={{
              backgroundColor: !category ? '#232e4e' : 'transparent',
              color: !category ? 'white' : '#232e4e',
              border: !category ? '1px solid #232e4e' : '1px solid #e5e7eb',
            }}
          >
            All
          </a>
          ...
          {categories.map((cat) => (
              ...
              className="px-4 py-2 rounded-full text-sm font-semibold transition hover:opacity-75"
              style={{
                backgroundColor: category === cat ? '#232e4e' : 'transparent',
                color: category === cat ? 'white' : '#232e4e',
                border: category === cat ? '1px solid #232e4e' : '1px solid #e5e7eb',
              }}
            >
              {cat}
            </a>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {(posts as any[]).map((post) => (
                    ...
                    key={post._id}
                    href={`/blog/${post.slug}`}
                    className="card hover:shadow-xl transition cursor-pointer block"
                  >
                    {/* Image */}
                    ...
                    {/* Title */}
                    <h3 className="font-bold text-xl mb-2 leading-7" style={{ color: '#232e4e' }}>
                      {post.title}
                    </h3>
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-2 mt-16">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                      ...
                      style={{
                        backgroundColor: p === page ? '#232e4e' : 'transparent',
                        color: p === page ? 'white' : '#232e4e',
                        border: p === page ? '1px solid #232e4e' : '1px solid #e5e7eb',
                      }}
                    >
                      {p}
                    </a>
