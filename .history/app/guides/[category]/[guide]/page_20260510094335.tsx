...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { category: categorySlug, guide: guideSlug } = await params
  const guide = await getGuideBySlug(categorySlug, guideSlug)
  if (!guide) return {}
  ...
  return {
    ...
    alternates: {
      canonical: `https://timmstravel.com/guides/${categorySlug}/${guideSlug}`,
    },
    openGraph: {
      title: guide.metaTitle,
      description: guide.ogDescription ?? guide.metaDescription,
      url: `https://timmstravel.com/guides/${categorySlug}/${guideSlug}`,
...
export default async function GuidePage({ params }: Props) {
  ...
  const articleSchema = {
    ...
    '@type': 'Article',
    '@id': `https://timmstravel.com/guides/${categorySlug}/${guideSlug}`,
    url: `https://timmstravel.com/guides/${categorySlug}/${guideSlug}`,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: guide.categoryTitle, item: `https://timmstravel.com/guides/${categorySlug}` },
      { '@type': 'ListItem', position: 4, name: guide.title, item: `https://timmstravel.com/guides/${categorySlug}/${guideSlug}` },
    ],
  }

  const relatedCategories = categories.filter((c) => c.slug !== categorySlug).slice(0, 3)
      ...
      <div className="max-w-4xl mx-auto px-6 pt-6">
        <BreadcrumbNav
          crumbs={[
            ...
            { label: guide.categoryTitle, href: `/guides/${categorySlug}` },
            { label: guide.title },
          ]}
        />
      </div>
