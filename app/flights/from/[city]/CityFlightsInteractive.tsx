...
export default function CityFlightsInteractive({
  city,
  airports,
  destinations,
}: {
    ...
    <>
      ...
      <section style={{ backgroundColor: '#232e4e' }} className="text-white py-20 px-6 text-center">
        ...
        <h1 className="text-4xl sm:text-5xl font-bold mb-4 leading-tight">
          Find Flights from {city}
        </h1>
        <p className="text-gray-300 mb-10 max-w-xl mx-auto">
          Compare fares from {airportNames} to destinations worldwide. No hidden fees, ever.
        </p>

        {destinations.length > 0 && selectedIata && (
          <CityFlightSearch
            originIATA={selectedIata}
            originCity={city}
      ...
      <section className="py-16 px-6 bg-gray-50 border-t border-gray-100">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-center text-gray-500 mb-10 max-w-2xl mx-auto">
            {airports.length > 1
              ? `${city} is served by ${airports.length} airports. Select one below to search flights from that airport.`
                ...
                >
                  ...
                  {isActive && (
                    <p className="text-xs mt-2 font-semibold" style={{ color: '#7dd3d6' }}>
                      Selected Airport - search updated ↑
                    </p>
                  )}
                </button>
              );
