...
export default function DealPageClient({ slug, categoryConfig }: DealPageClientProps) {
  ...
  const resolveLocationLabel = (iata: string, format: 'full' | 'short' = 'short') => {
    ...
    // 💡 1. Metacity exact mappings take priority to avoid lookup bugs
    ...
    // 💡 2. Standard precise JSON index search
    ...
    // 💡 3. For short/slug format, strictly prefer the City label name over the specific Airport runway name
    return foundAirport.city || foundAirport.name
  }
  ...
  useEffect(() => {
    async function fetchFilteredData() {
      try {
        ...
        let chosenOrigin = categoryConfig.origin

        if (!chosenOrigin) {
          if (slug.includes('barcelona') || slug === 'bcn') {
            chosenOrigin = 'BCN'
        ...
        const isUnder50Global = slug === 'under-50'
        const isEuropeUnder50 = slug === 'europe-under-50'
        const operationalMaxPrice = (isUnder50Global || isEuropeUnder50) ? 50 : categoryConfig.maxPrice
        const limit = (slug === 'ski-deals' || slug === 'christmas-deals') ? 1000 : 500
        
        const response = await fetch(`/api/trending-deals?origin=${chosenOrigin}&limit=${limit}&slug=${slug}&_t=${Date.now()}`)
        
        if (response.ok) {
          const data = await response.json()
          const rawDeals: TrendingDeal[] = data.deals || []

          const processed = rawDeals.filter(deal => {
            if (operationalMaxPrice && deal.price > operationalMaxPrice) return false
            ...
            if (slug === 'christmas-deals' && month === 11 && day < 23) {
              return false
            }
        ...
        console.error('Failed processing data array', err)
      } finally {
        setLoading(false)
      }
    }
          ...
          <div className="max-w-6xl mx-auto">
            {loading ? (
              <div className="text-center py-20">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 mx-auto mb-4" style={{ borderColor: '#03989e' }} />
                <p className="text-sm text-gray-500">Scanning real-time flight records cache...</p>
              </div>
            ) : deals.length === 0 ? (
                        ...
                        <div className="space-y-2 border-t border-gray-200/50 pt-3 text-xs text-gray-500 mb-6">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-semibold text-gray-700">
                              {displayFormatDate(deal.departDate)}
                            </span>
                          </div>
                          {deal.returnDate && (
                            <div className="flex justify-between items-center">
                              ...
                              <span className="font-semibold text-gray-700">
                                {displayFormatDate(deal.returnDate)}
                              </span>
                            </div>
                          )}
                        </div>
