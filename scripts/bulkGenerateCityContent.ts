...
// Run with: npx ts-node --skip-project scripts/bulkGenerateCityContent.ts
...
// -----------------------------
async function getAllCities(): Promise<SanityCity[]> {
  ...
  const data = await response.json() as SanityQueryResult
  return data.result
}
...
async function main() {
  ...
  for (let i = 0; i < toGenerate.length; i++) {
    ...
    try {
      ...
      console.error(`  ❌ Failed: ${cityName} — ${message}`)
