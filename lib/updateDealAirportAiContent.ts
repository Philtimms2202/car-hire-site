import type { DealAirportAiContent } from './generateDealAirportAiContent'

export function buildDealAirportDocId(categorySlug: string, airportSlug: string): string {
  return `dealAirport-${categorySlug}-${airportSlug}`
}

export async function updateDealAirportAiContent(
  categorySlug: string,
  airportSlug: string,
  categoryTitle: string,
  airportLabel: string,
  content: DealAirportAiContent,
  factsUsed: string[]
): Promise<void> {
